...
if (!drawer) {
  ...
  examContainer.insertBefore(drawer, examForm);
      ...
      questionPool.forEach((q, idx) => {
        ...
        div.innerHTML = `<p><strong>Q${idx+1}:</strong> ${q.question_text}</p>`;
        ...
        // Mark checkbox (restore checked state)
...
if (uniqueSubtopics.length) {
  ...
  const perSubNeeded = Math.max(minPerSubtopic, Math.ceil(numQuestions / uniqueSubtopics.length));
...
const minPerSubtopic = 2; // tune as you like
const examQ = pickBalanced(bySub, numQuestions, minPerSubtopic);
questionPool = examQ;
 ...
 examQ.forEach((q, idx) => {
  ...
  // store identifiers for the drawer to use
  div.dataset.qid = q.id;
  div.dataset.qnum = String(idx + 1);
...
if (q.type === 'open') {
  ...
  if (authErr || !user) {
    ...
    // 4. Insert into user_exam_results
    const payload = {
      user_id:           user.id,
      ...
      topic_id:          currentSubtopics[0], // assuming first subtopic is representative
    };
    // wrap in array form
    const { data, error: insertErr } = await supabase
      .from('user_exam_results')
      .insert([ payload ])
      .select();    // <-- ask for the row back so you can inspect it

  if (insertErr) {
    console.error('✖️ Failed to record exam result:', insertErr);
    return;
  }
  console.log('✅ Exam result recorded (if any):', data);
...
for (let idx = 0; idx < questionPool.length; idx++) {
  ...
  let isCorrect = false;
  if (q.type === 'open') {
    isCorrect = chosen ? isCorrectOpen(q, chosen) : false;
  } else {
  ...
  items.push({
    ...
    time_ms: null, // you can add timing later if you want
  });
}
...
if (items.length && examResultId) {
  ...
  // Hide exam UI and header; show results with a Save button
  ...
  if (examResultId) {
    ...
    saveBtn.addEventListener('click', async () => {
      try {
        ...
        // Try to persist items too
        if (examResultId && items.length) {
          const { error: itemsErr2 } = await supabase.from('exam_items').insert(items.map(it => ({
            ...it, exam_result_id: examResultId
          })));
 ...
 reportEl.innerHTML = '';            // clear old
 questionPool.forEach((q, idx) => {
const radioEl  = document.querySelector(`input[name="q${idx}"]:checked`);
const textEl   = document.querySelector(`input[name="q${idx}"]`);
const chosen   = radioEl ? radioEl.value : (textEl ? textEl.value.trim() : '[no answer]');
const correct  = q.type === 'open' ? (q.answer_text ?? '') : q.correct_option;
   ...
   block.innerHTML = `
     ...
     <p>Your answer: <em>${chosen}</em></p>
     <p>Correct answer: <em>${correct}</em></p>
     <details class="mt-2">
       <summary class="cursor-pointer text-blue-600">Explanation</summary>
       <p class="mt-1">${q.explanation || 'No explanation provided.'}</p>
     </details>
   `;
renderMath(document.getElementById('examForm'));
