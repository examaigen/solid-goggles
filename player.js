...
function unescapeKaTeXString(s){
  ...
  // 1) General: \\ followed by ( ) [ ] { } \ or a letter -> collapse to \
  s = s.replace(/\\\\(?=[(){}\[\]\\A-Za-z])/g, '\\');
  // 2) Optional: collapse triple to single in edge cases like "\\\(" (rare)
...
async function loadRandom(){
  ...
  try {
    ...
    let q = supabase.from('questions').select('*').in('subtopic_id', subtopicIds).limit(50);
    if (selectedClass) q = q.eq('class', selectedClass);
    const { data, error } = await q;
    if (error) throw error;
    if (my !== loadSeq) return;
    if (!data?.length){ questionEl.textContent = "Nėra klausimų (patikrinkite pasirinkimus)."; return; }
...
input.addEventListener("input", () => {
  ...
  checkBtn.classList.toggle('cursor-not-allowed', !ready);
    ...
    // Normalize both formats and unescape KaTeX inside choices
    ...
    const ul = document.createElement("div");   // ← create the container
    ...
    Object.entries(opts).forEach(([letter, text]) => {
      ...
      label.innerHTML = `
        <input type="radio" name="choice" value="${letter}" id="${id}" class="mr-2">
